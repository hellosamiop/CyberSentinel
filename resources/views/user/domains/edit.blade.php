    ...
    <div class="py-20 mx-auto max-w-7xl">
        <div class="container mx-auto px-4">
            <div class="mx-auto bg-white p-6 shadow" style="border-radius: 15px">
                ...
                @endif
                <h2 class="text-2xl font-bold mb-4">Edit Domain</h2>
                    ...
                    <div class="flex gap-6 flex-col sm:flex-row">
                        <div class="w-full">
                            <div class="mb-4">
                                ...
                                <input type="text" class="mt-1 p-2 w-full border" style="border-radius: 15px" name="name" id="name" value="{{ $domain->name }}" >
                            </div>
                            <div class="mb-4">
                                <label for="domain_url" class="block text-sm font-semibold text-gray-600">Domain URL:</label>
                                <input type="url" class="mt-1 p-2 w-full border"  style="border-radius: 15px" name="domain_url" id="domain_url" value="{{ $domain->domain_url }}" required>
                            </div>
                            <div class="mb-4">
                                ...
                                <select name="industry_id" id="industry_id" class="mt-1 p-2 w-full border"  style="border-radius: 15px" required>
                                    @foreach($industries as $industry)
                                        <option value="{{ $industry->id }}" {{ $domain->industry_id == $industry->id ? 'selected' : '' }}>{{ $industry->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-4">
                                ...
                                <select name="country_id" id="country_id" class="mt-1 p-2 w-full border"  style="border-radius: 15px" required>
                                    @foreach($countries as $country)
                                        <option value="{{ $country->id }}" {{ $domain->country_id == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="w-full">
                            <div class="mb-3" style="border-radius: 15px; width: 100%;">
                                <img id="logo_preview" class="border"
                                     src="{{ $domain->logo }}"
                                     style="border-radius: 15px;width: 100%; height: 250px;" />
                            </div>
                            <div class="mb-4">
                                <label for="logo" class="block text-sm font-semibold text-gray-600">Logo:</label>
                                <input type="file" class="mt-1 p-2 w-full border" style="border-radius: 15px" name="logo" id="logo" accept="image/*">
                            </div>
                        </div>
                    </div>
                    <div class="flex justify-end items-center space-x-4">
                        <button type="submit" class="bg-wave-500 hover:bg-wave-700 text-white font-bold py-2 px-4 rounded">
                            Update
                        </button>
