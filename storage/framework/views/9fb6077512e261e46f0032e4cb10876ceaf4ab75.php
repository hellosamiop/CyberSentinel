...
<div id="announcement" class="fixed bottom-0 right-0 z-50 flex items-end justify-center px-4 py-6 transition-all duration-200 ease-out transform translate-y-0 opacity-100 cursor-pointer pointer-events-none hover:-translate-y-1 sm:p-6 sm:items-start sm:justify-end" data-href="<?php echo e(route('wave.announcement', $announcement->id)); ?>">
	<div id="announcement_content" class="relative w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-lg pointer-events-auto">
		<div class="relative p-5">
			...
			<h4 class="text-sm font-medium leading-5 text-gray-900"><?php echo e($announcement->title); ?></h4>
			<p class="mt-2 text-sm leading-5 text-gray-500"><?php echo e($announcement->description); ?></p>
			<div id="announcement_footer" class="mt-1"><a href="<?php echo e(route('wave.announcement', $announcement->id)); ?>" class="text-sm font-medium leading-5 text-indigo-600 transition duration-150 ease-in-out hover:text-indigo-500 focus:outline-none focus:underline">Learn More</a></div>
		</div>
	</div>
</div>
...
<script>
	...
	function markNotificationsRead(endpoint, splitPopReadyState){
		...
		HttpRequest.open("POST", "<?php echo e(route('wave.announcements.read')); ?>", true);
		HttpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		HttpRequest.send("_token=<?php echo e(csrf_token()); ?>");
	}
</script>
<?php /**PATH D:\FYP\cybersentinel\resources\views/themes/tailwind/partials/announcements.blade.php ENDPATH**/ ?>